 ...
 * object is present, else `global.PB = API`. We load its source and evaluate
...
function loadEngine() {
  ...
  const factory = new Function(
    "module",
    "globalThis",
    source + "\n;return (module.exports && module.exports.Calc) ? module.exports : globalThis.PB;"
  );
  return factory(mod, {});
}
...
test("exports the expected public API", () => {
  for (const name of ["SCALAR", "LINEAR", "AREA", "VOLUME", "Calc", "combine", "fold", "fmtFtIn"]) {
    assert.ok(name in PB, `missing export: ${name}`);
  }
});

test("formats inches as feet-inch-fraction", () => {
  assert.equal(PB.fmtFtIn(12.5, 16), "1' 1/2\"");
  assert.equal(PB.fmtFtIn(100, 16), "8' 4\"");
  assert.equal(PB.fmtFtIn(0, 16), '0"');
  assert.equal(PB.fmtFtIn(-6, 16), '-6"');
});

test("reduces fractions to lowest terms", () => {
  // 8/16 -> 1/2, 12/16 -> 3/4
  assert.equal(PB.fmtInchesOnly(0.5, 16), '1/2"');
  assert.equal(PB.fmtInchesOnly(0.75, 16), '3/4"');
});

test("adds feet and inches", () => {
  // 12'6" + 3'8" = 16'2"
  const c = calc(["1", "2", "ft", "6", "in", "+", "3", "ft", "8", "in", "="]);
  assert.equal(PB.fmtFtIn(c.result().acc.n, 16), "16' 2\"");
});

test("length x length promotes to area", () => {
  const acc = calc(["3", "ft", "*", "4", "ft", "="]).result().acc;
  assert.equal(acc.dim, PB.AREA);
...
test("area x length promotes to volume", () => {
  ...
  assert.equal(acc.dim, PB.VOLUME);
...
test("multiplying past volume errors instead of overflowing dimension", () => {
  const r = PB.combine({ n: 1, dim: PB.VOLUME }, "*", { n: 2, dim: PB.LINEAR });
  assert.ok(r.error);
});

test("divide by zero errors", () => {
  const r = PB.combine({ n: 5, dim: PB.SCALAR }, "/", { n: 0, dim: PB.SCALAR });
  assert.ok(r.error);
});
...
test("circle derives circumference and area from a diameter", () => {
  ...
  assert.equal(diam.dim, PB.LINEAR);
  assert.equal(circ.dim, PB.LINEAR);
  assert.equal(area.dim, PB.AREA);
...
test("sqrt of an area returns a length", () => {
  ...
  const out = c.sqrt();
  assert.equal(out.dim, PB.LINEAR);
...
test("metric formatting switches mm/m by magnitude", () => {
  assert.ok(PB.fmtMetric({ n: 39.37007874015748, dim: PB.LINEAR }).endsWith("m"));
  assert.ok(PB.fmtMetric({ n: 1, dim: PB.LINEAR }).endsWith("mm")); // < 1 m
});
